...
Die Erhebung der Testdaten erfolgt durch das \textit{test.py} Skript, die Auswertung durch das \textit{experiment\_evaluation.py} Skript.

Mit der Ausführung des ersten Skripts wird ein Testklasse instanziiert und darin enthalten eine Simulationsobjekt nach den Eigenschaften des Testszenarios erzeugt.
Die Bestimmung des zufälligen Zielpunktes folgt ebenso wie die Initialisierung der Verteidigerdrohne einer uniformen Verteilung bis zu 1,5 Meter um den Ursprung.
Eine uniforme Verteilung fördert dabei die Varianz der Testszenarien und erhöht so die Validität des Experiments.
Anders als bei der Initialisierung der Drohnen wird die Höhe des Zielpunktes stets auf 0,5 Meter festgelegt.
...
Zu jedem Zeitschritt jeder Episode wird die das RL-Modell bestimmte Aktion ausgeübt und eine Teilmenge dessen Resultats gemessen.
...
Beide Werte werden anschließend mit der aktuellen Episode und dem Zeitschritt einer Tabelle als Datensatz hinzugefügt.
Nach dem erfolgreichem Durchlaufen aller Zeitschritte aller 100 Episoden wird die Tabelle als Komma separierte Wertedatei (CSV) gespeichert.
...
Nach dem Laden und Vorverarbeiten entspricht der Aufbau der Messdaten zweier Tabellen folgender Struktur.
...
Anschließend wird je Metrik der entsprechende Test zur Ungleichheit und Verbesserung ausgeführt, woraus je Test eine Teststatistik und ein P-Wert gespeichert wird.
...
Die Auswertung dieser Metriken aus dem Vergleich der Leistungsdaten zweier Strategien in den Testszenarien wird zweimal durchgeführt.
Dabei werden die Leistungsunterschiede zwischen der Strategien aus Trainingsszenario eins und drei sowie aus Szenario eins und vier betrachtet.
Abbildung X zeigt nachfolgend den Prozess des Laborexperiments als BPMN Modellierung auf. 
