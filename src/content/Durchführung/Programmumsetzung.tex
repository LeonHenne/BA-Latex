...
Die Umsetzung der Anforderungen der Simulationsumgebung beinhaltet im Fokus die Entwicklung einer Simulationsumgebung anhand des Gymnasium-Programmiergerüsts. % Im Kern der Implementierung der Simulationsumgebung liegt das Gymnasium-Programmergerüst.
...
Weiterhin stellt die Neuentwicklung einer Simulationsumgebung kein Entwicklungsaufwand dar, welcher einen Einfluss auf die auszuwertende Forschungsfrage ausübt. % Nicht klar, was gemeint ist. Wieso hat der Entwicklungsaufwand einen Einfluss auf die auszuwertende Forschungsfrage?
...
Zum Anderen sollte die Simulation einen beherrschbaren Rechenleistungsaufwand verursachen, da die Zielsetzung die Optimierung mehrerer Policies beinhaltet. % da die Zielsetzung dieser Arbeit die Optimierung mehrerer Policies voraussetzt
...
Demnach wird für die nachfolgende Entwicklung der eigenen Simulation auf der gym-pybullet-drones Bibliothek aufgebaut. % Entwicklung der für diese Arbeit verwendeten Simulation
...
Zusätzlich kann auch ein PID-Kontroller eingesetzt werden um die Steuerungssignale entgegenzunehmen. % PID-Regler
Zur Entwicklung der eigenen Simulation wird die Steuerung über einen Richtungsvektor und einen Geschwindigkeitswert gewählt. % Entwicklung der für diese Arbeit verwendeten Simulation
Unabhängig des gewählten Aktionstyps werden alle Steuerungssignale durch Kontrollerklassen in konkrete Drehzahlen übersetzt. % Reglerklassen
...
Die Umsetzung, der zu Beginn dieses Kapitels beschriebenen Simulationsumgebung, basiert auf der Verwendung der Basissimulation und der Kontrollerklassen. % Kontrollerklasse ist glaube ich kein deutscher Begriff - Reglerklasse oder Steuerungsklasse? Oder in denglisch Controller-Klasse
...
Kernaufgabe dieser Funktion ist die Umsetzung der Eigenschaften eines Trainings- und Testszenarios. % Du meinst hier nicht die Belohnungsfunktion sondern die Init-Funktion oder? Dann müsstest du "Kernaufgabe der Funktion" schreiben; "dieser Funktion" bezieht sich strenggenommen auf die zuletzt genannte Funktion
...
% entspricht dies dem reduzierten Beobachtungsraum?

% Action Space
Die Definition des \textbf{Aktionsraums} ist in der ActionSpace Funktion bestimmt. % die Init-Funktion ist fett hervorgeheben, die ActionSpace-Funktion nicht und für den Observation space wurde gar keine Funktion angegeben
Je nach Aktionstyp ist ein drei oder vierdimensionaler Vektorbereich definiert. % es wird nur ein Aktionstyp unterstützt - daher würde ich nur auf den verwendeten Aktonsraum eingehen
...
Die zuvor beschriebene Klasse vererbt ihre Variablen und Funktionen auf die höhergelegenen Abstraktionsschichten der \textbf{DefenderDroneWrapper} und \textbf{AttackerDroneWrapper} Klasse. % an die niedrigliegendere Abstraktionsschichten
...
Auf der Ebene der Wrapper-Klassen wird der \textbf{Beobachtungsraum} mit allen Informationen beider Quadrokopter zu einem 40-elementigen Vektor zusammengefasst. % das entspricht nicht dem reduzierten Beobachtungsraum oder?
...
Innerhalb einer Trainingsiteration wird die Simulation unter den Trainingsgegebenheiten in 20000 Episoden ausgeführt. % wir haben letztendlich nur eine Iteration durchgeführt, daher besser: Für die Erhebung der Trainingsdaten wird zunächst die Simulation unter den Trainingsgegebenheiten in 20000 Episoden ausgeführt
Beim imitierenden Lernen wird in der Trainingsumgebung eine gewählte Aktion für 50 Zeitschritte ausgeführt, um die Trainingsdaten aus einer Episode zu minimieren. % Für das imitierende Lernen wurde der Parameter AGGR_PHY_STEPS der BaseAviary Klasse auf 50 gesetzt, was bedeutet, dass die Schrittfunktion nur alle 50 Zeitschritte aufgerufen werden und so je Episode deutlich weniger Aktionen erfolgen und Transitionen aufgezeichnet werden.
...
Die Anpassung der Gewichte einer Akteur-Kritiker-Strategie wird durch den Behavioral Cloning Algorithmus vorgenommen. % Hier könnte noch ein Satz wie "Ziel des dahinterliegenden neuronalen Netzes ist es eine Funktion von einem beobachteten Zustand zu der vom Experten gewählten Aktion zu approximieren." kommen
Insgesamt wird dieser abwechselnde Vorgang der Erhebung von Daten und die zur Basis dessen stattfindende Optimierung so häufig wiederholt, bis alle Iterationen die Menge an Expertendemonstrationen chargenweise abgearbeitet haben. % haben wir letztendlich nicht getan, würde daher den Satz entfernen

Als Experte wird eine regelbasierte Version der zu optimierenden Drohne eingesetzt. % Als Experte wird eine regelbasierte Version fder jeweils zu optimierenden Strategie eingesetzt.
...
Im RL-Prozess ist zu beachten, dass anders als beim imitierenden Lernen eine gewählte Aktion lediglich einen Zeitschritt ausgeführt wird, da hierbei keine Trainingsdaten gespeichert werden müssen. % dass anders als beim imitierenden Lernen zu jedem Zeitschritt eine neue Aktion gewählt wird, da der Parameter AGGR_PHY_STEPS hier standardmäßig bei 1 liegt
Mit dem Einsatz des \textit{train.py} Programms lässt sich das in einer Pickle-Datei gespeicherte Modell laden.
Dazu wird das grundlegende neuronale Netz des PPO-Objekts mit derselben Struktur der Modells aus dem imitierenden Lernen initialisiert. % mit derselben Architektur und den trainierten gewichten des Modells aud dem imitierenden Lernen initialisiert
...
Kernaufgabe dieses Teils ist die Erhebung und Auswertung der Messdaten zu dem in der Einleitung des Kapitels verfassten Experimentaufbau. % entsprechend des in der Einleitung des Kapitels verfassten Experimentaufbaus
...
Zu jedem Zeitschritt jeder Episode wird durch das RL-Modell eine bestimmte Aktion ausgeübt und eine Teilmenge des Resultats gemessen. % oder auch: eine Teilmenge des daraus resultierenden Zustands
...
Beide Werte werden mit der aktuellen Episode und dem Zeitschritt einer Tabelle als Datensatz hinzugefügt. % Beide Werte werden gekennzeichnet mit der jeweiligen Episode und Zeitschritt einer Tabelle als Datensatz hinzugefügt.
...
Die erstellte Datei wird anknüpfend mittels \textit{experiment\_evaluation.py} ausgewertet, indem die Messdaten geladen, die Signifikanztests durchgeführt und daran die Hypothesen bestimmt werden. % und jeweils ein Wahrheitswert für die Annahme/Ablehnung der Hypothesen bestimmt wird
...
Dabei wird zunächst die Episodenlänge, die kumulierte Belohnung und die Anzahl an Misserfolgen ermittelt. % Anzahl an Misserfolgen je Episode? Eine Episode kann nach meinem Verständnis nur einen Misserfolg oder einen Erfolg darstellen
Zu dieser Basis kann die Abweichung der Belohnung je Episode zu dem Mittel aller kumulierten Belohnungen im Testprozess berechnet werden. % Basierend darauf
...
Dabei werden die erzielten Messdaten zwischen den Strategien aus Trainingsszenario eins und drei sowie aus Szenario eins und vier betrachtet. % betrachtet oder verglichen?
