...
Methoden basierend auf diesem Wertigkeitswert lernen eine Schätzfunktion der Wertigkeit für alle Zustände ($V_{\pi}(s) \forall S$) und alle Zustandsaktions-Paare ($Q_{\pi}(s_{t},a_{t}) \forall s,a \in (S,A)$) der optimalen Strategie $\pi^{*}$ durch aktualisieren der folgenden Funktionen eins und zwei: \footcite[Vgl.][S. 2]{Zhang.2018} % Wird nur die Wertigkeit der Zustandsaktionspaare der optimalen Strategie gelernt oder aller Zustandsaktionspaare? Ich dachte eigentlich Letzteres. Dann sollte der Satzteil 'der optimalen Strategie $\pi^{*}$' entfernt werden.
\begin{description}
...
Als wertbasierte Methode wird für jeden Zustand die Wertigkeit berechnet, was unter DQL mittels eines neuronalen Netzes erreicht wird, welches die Q-Funktion darstellt. \footcite[Vgl.][S. 4]{Huang.2017} %darstellt oder approximiert?
...
Erfahrungswiederholung beschreibt die Technik die Erfahrung des Agenten, also in welchem Zustand welche Aktion zu welcher Belohnung und welchem Folgezustand führt, in jedem Zeitschritt zu speichern und zufällig anhand neuer Erfahrung anzupassen. \footcite[Vgl.][S. 4]{Mnih.2013} % Werden hier tatsächlich gespeicherte Erfahrungen angepasst? Mein Verständnis ist, dass beim Training zufällig auf gespeicherte Zustansübergänge zugreift und diese damit auch aus der festen und stark korrelierten Abfolge in dem tatsächlichen Verlauf einer Episode nimmt. Die Updates/Anpassungen beziehen sich dagegen auf die Gewichte der Q-Funktion.
...
Aus der unterschiedlichen Interaktion jedes Agenten mit der selben Umgebung ergibt sich der Unterschied, dass die Umgebungsdynamik aus der Kombination aller Aktionen der Agenten beeinflusst wird anstatt aus der Aktion des einzelnen Agenten. \footcite[Vgl.][S. 2]{Wong.2022} % 'Aus der unterschiedlichen Interaktion' -> 'Aus den einzelnen Interaktionen'
Da dieser Effekt auch die Annahme der Stationarität von Markov Entscheidungsprozessen verletzt, bedarf die Umgebung auch einer anderen Representation. \footcite[Vgl.][S. 6]{Wong.2022} % zweimal 'auch' in einem Satz, das erste 'auch' kann mMn weggelassen werden
...
% Hier sollte noch ein Satz zu der Einordnung deines Szenarios kommen, z.B. "Das für die Experimente vorgesehene Szenario lässt sich demnach in das kompetitive Multi-Agent Reincorfement Learning einordnen."
